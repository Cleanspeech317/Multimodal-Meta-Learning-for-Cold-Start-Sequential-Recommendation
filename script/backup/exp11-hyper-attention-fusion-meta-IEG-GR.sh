...
sasrec_meta_train_log=$up_log_dir/SASRec.item-emb-gen.gr-0.2.log
...
do
    ...
    do
        meta_train_log=$up_log_dir/attention-fusion.meta-train.IEG.$attention_learning_rate."$mlp_hidden_size".gr-0.2.log
        meta_test_log=$down_log_dir/attention-fusion.meta-test.IEG.$attention_learning_rate."$mlp_hidden_size".gr-0.2.log
        
        attention_module_file="pth/$city-$attention_learning_rate-$mlp_hidden_size-meta-IEG-attention_module.pth"
        echo $meta_train_log

        python run_attention_fusion_train.py --attention_learning_rate=$attention_learning_rate --mlp_hidden_size="$mlp_hidden_size" --attention_module_file="$attention_module_file" --dataset=$city --data_source=up --model_list="$model_list" --config_files="$meta_test_config_files" --model_file="['$sasrec_meta_file','$txt_meta_file','$img_meta_file']" --fusion_weight="$fusion_weight" --gpu_id=$gpu_id --hint="meta attention_fusion_train for $city" &> $meta_train_log &
        wait

        python run_meta_fusion_test.py --attention_learning_rate=$attention_learning_rate --mlp_hidden_size="$mlp_hidden_size" --attention_module_file="$attention_module_file" --dataset=$city --model_list="$model_list" --config_files="$meta_test_config_files" --model_file="['$sasrec_meta_file','$txt_meta_file','$img_meta_file']" --gpu_id=$gpu_id --hint="meta $city fusion test" &> $meta_test_log &
        wait
    done
done
