...
log_dir=output/$city/new_user_$split_ratio/up-10-inf
...
python run_recbole.py --dataset=pretrain --model=SASRec $base_yaml_file --val_interval="{'cityid': ['$city_id']}" --data_source=up --time_split_ratio=$split_ratio --epochs=100 --eval_args="{'split': {'LS': 'test_only'}, 'order': 'TO', 'mode': 'full', 'group_by': 'user'}" --gpu_id=$gpu_id --hint="$city up" &> $sasrec_up_log &
sleep 5
...
# sleep 5
python run_meta_train.py --dataset=pretrain --model=SASRec $meta_yaml_file --val_interval="{'cityid': ['$city_id']}" --data_source=up --time_split_ratio=$split_ratio --gpu_id=$gpu_id --meta_epochs=10 --num_local_update=5 --hint="meta $city train" &> $meta_train_log &
wait
