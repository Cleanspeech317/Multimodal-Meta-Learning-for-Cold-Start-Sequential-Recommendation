# $1 model  $2 city  $3 up_lr_list  $4 down_lr_list  $5 generate_rate_list  $6 gpu_id  $7 type  $8 replace

model=$1
city=$2
up_lr_list=$3
down_lr_list=$4
generate_rate_list=$5
gpu_id=$6
type=$7
replace=$8
...
do
    ...
    do
        item_emb_gen_log=$up_log_dir/$model.item-emb-gen.${generate_rate}-rate.log
        if [[ ( "$type" != "trans" ) && ( ! -f $item_emb_gen_log || "$replace" == "replace" ) ]]
        then
            python run_gen_item_emb.py --learning_rate=$up_lr --generate_rate=$generate_rate --dataset=$city --model=$model $item_emb_gen_yaml_file --model_file=$origin_meta_file --gpu_id=$gpu_id --hint="meta $city train" &> $item_emb_gen_log &
        fi
        wait
        ...
        do
            meta_test_log=$down_log_dir/$model.meta-test.$down_lr.IEG.${generate_rate}-rate.log
