...
python run_recbole.py --dataset=pretrain --model=SASRec $base_yaml_file --val_interval="{'cityid': ['$city_id']}" --data_source=down --downstream_user_inter_num_interval=$down_interval --epochs=100 --gpu_id=$gpu_id --hint="$city down" &> $sasrec_down_log &
sleep 10
python run_meta_test.py --dataset=pretrain --model=SASRec $meta_yaml_file --val_interval="{'cityid': ['$city_id']}" --data_source=down --downstream_user_inter_num_interval=$down_interval --model_file=$meta_file --epochs=50 --gpu_id=$gpu_id --hint="meta $city test" &> $meta_test_log &
sleep 10
python run_meta_test.py --dataset=pretrain --model=SASRec $meta_yaml_file --val_interval="{'cityid': ['$city_id']}" --data_source=down --downstream_user_inter_num_interval=$down_interval --model_file=$trans_file --epochs=50 --gpu_id=$gpu_id --hint="trans $city" &> $trans_log &
wait
