...
item_emb_gen_log=$up_log_dir/SASRec.item-emb-gen.nn-10.log
if [[ ! -f $item_emb_gen_log || "$replace" == "replace" ]]
then
    python run_gen_item_emb.py --item_neighbour_num=10 --model=SASRec --dataset=$city $item_emb_gen_yaml_file --model_file=$meta_file --gpu_id=$gpu_id --hint="meta $city train" &> $item_emb_gen_log &
fi
wait

new_meta_file=`tail $item_emb_gen_log -n 3 | head -n 1 | awk '{ print $9 }'`
new_meta_test_log=$down_log_dir/SASRec.meta-test.IEG.log
