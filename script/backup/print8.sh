...
up_log_dir=output/$city/0121-exp-1e-3/up-${up_interval}
...
meta_test_log=$down_log_dir/$model.meta-test.1e-3.log
trans_log=$down_log_dir/$model.trans.1e-3.log
...
item_num=`grep 'The number of items:' $trans_log | awk '{ print $5 }'`
inter_num=`grep 'The number of inters:' $trans_log | awk '{ print $5 }'`
user_inter_num=`grep 'Average actions of users:' $trans_log | awk '{ printf("%.4f", $5) }'`
item_inter_num=`grep 'Average actions of items:' $trans_log | awk '{ printf("%.4f", $5) }'`
