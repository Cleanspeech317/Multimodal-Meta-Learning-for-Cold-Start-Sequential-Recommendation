...
item_emb_gen_log=$up_log_dir/SASRec.item-emb-gen.log
if [[ ! -f $item_emb_gen_log || "$replace" == "replace" ]]
then
    python run_gen_item_emb.py --model=SASRec --dataset=$city $item_emb_gen_yaml_file --model_file=$meta_file --gpu_id=$gpu_id --hint="meta $city train" &> $item_emb_gen_log &
fi
wait
