# $1 model  $2 city  $3 up_lr_list  $4 down_lr_list  $5 item_neighbour_num_list  $6 generate_rate_list  $7 gpu_id  $8 type  $9 replace
...
do
    ...
    do
        ...
        do
            item_emb_gen_log=$up_log_dir/$model.item-emb-gen-mean.${item_neighbour_num}-neighbour.${generate_rate}-rate.log
            if [[ ( "$type" != "trans" ) && ( ! -f $item_emb_gen_log || "$replace" == "replace" ) ]]
            then
                python run_gen_item_emb.py --learning_rate=$up_lr --item_neighbour_num=$item_neighbour_num --generate_rate=$generate_rate --dataset=$city --model=$model $item_emb_gen_yaml_file --model_file=$origin_meta_file --gpu_id=$gpu_id --hint="meta $city train" &> $item_emb_gen_log &
            fi
            wait
            ...
            do
                meta_test_log=$down_log_dir/$model.meta-test.$down_lr.IEGM.${item_neighbour_num}-neighbour.${generate_rate}-rate.log
